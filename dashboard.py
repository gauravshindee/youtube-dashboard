...
DOWNLOADED_SHEET = "already downloaded"
...
def load_downloaded():
    try:
        return load_sheet(DOWNLOADED_SHEET).get_all_records()
    except gspread.exceptions.WorksheetNotFound:
        return []

def log_to_downloaded(video):
    try:
        sh = gs_client.open_by_key(GOOGLE_SHEET_ID)
        try:
            sheet = sh.worksheet(DOWNLOADED_SHEET)
        except gspread.exceptions.WorksheetNotFound:
            sheet = sh.add_worksheet(title=DOWNLOADED_SHEET, rows="1000", cols="5")
        ...
        st.error(f"❌ Failed to save to downloaded tab: {e}")
            ...
            if st.button("⬇️ Download", key=f"dl_{video['link']}"):
                with st.spinner("Downloading..."):
                    ...
                    if path and fname and vid:
                        log_to_downloaded(video)
                        with open(path, "rb") as f:
                            st.download_button("📥 Download to your device", data=f, file_name=fname, mime="video/mp4")
